...
public class AppMenuListener extends DefaultActionMenuListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equalsIgnoreCase("ChangePassword")) {
					...
					b.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							if (fChangePassword.isEmpty("Password") || fChangePassword.isEmpty("ConfirmPassword")) {
								fChangePassword.message(dChangePassword, "insert_new_pass_and_confirm",
										Form.ERROR_MESSAGE);
								return;
							} else {
								if (fChangePassword.getDataFieldValue("Password")
										.equals(fChangePassword.getDataFieldValue("ConfirmPassword")) == false) {
									fChangePassword.message(dChangePassword, "Password don't match",
											Form.ERROR_MESSAGE);
									return;
								}
								try {
									String loginEntity = ((UtilReferenceLocator) application.getReferenceLocator())
											.getLoginEntityName(application.getReferenceLocator().getSessionId());
									...
									EntityResult res = eUsu.update(av, cv,
											application.getReferenceLocator().getSessionId());
									System.out.println(res);
									if (res.getCode() == EntityResult.OPERATION_WRONG) {
										fChangePassword.message(dChangePassword, res.getMessage(), Form.ERROR_MESSAGE);
									} else {
										fChangePassword.message(dChangePassword, "Password changed successfully",
			...
			fChangePassword.setDataFieldValue("User_",
					((ReferenceLocator) application.getReferenceLocator()).getUser());
